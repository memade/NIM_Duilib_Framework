...
namespace ui
{
 ...
 std::shared_ptr<ImageInfo> GlobalManager::GetImage(const std::wstring& bitmap)
 {
  ...
  if (it == m_mImageHash.end()) {
   ...
   {
#if 0//!@ 格式化路径
    for (auto it = imageFullPath.begin(); it != imageFullPath.end(); ++it) {
     if (*it == '\\' || *it == '/')
      *it = '/';
    }
#endif
 ...
 bool GlobalManager::OpenResZipBuffer(const std::string& buffer, const std::string& password = "") {
  bool result = false;
  do {
   if (buffer.empty())
    break;
   ...
   g_hzip = OpenZip((void*)buffer.data(), buffer.size(), password.c_str());
   if (!g_hzip)
    break;
   result = true;
  } while (0);
